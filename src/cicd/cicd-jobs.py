...
def parse_args():
    ...
    parser.add_argument("DOMINO_JOB_OP", type=str, help="Operation: create, list, or update.")
    ...
    parser.add_argument("DOMINO_JOB_COMMAND", type=str, help="Command to run for the job. Command format: 'main.py arg1 arg2'.")
    ...
    parser.add_argument("DOMINO_JOB_ENVIRONMENT_ID", type=str, help="Environment Id of the Job.")
...
def get_hardware_tier_id(domino_url, user_api_key, hardware_tier_name):
    ...
    tier_id = next((tier["id"] for tier in hardware_tier_list if tier["name"] == hardware_tier_name), None)
    return tier_id

def job_start(start_job_url, project_id, command, commit_id, hardware_tier_name, environment_id, user_api_key):
    payload = {
...
def main():
    ...
    project_id = get_project_id(inputs.DOMINO_API_HOST, project_name, inputs.DOMINO_USER_API_KEY)

    if inputs.DOMINO_JOB_OP == "start":
        job_start(start_job_url, project_id, inputs.DOMINO_JOB_COMMAND, inputs.DOMINO_JOB_COMMIT_ID, inputs.DOMINO_JOB_HARDWARE_TIER_NAME, inputs.DOMINO_JOB_ENVIRONMENT_ID, inputs.DOMINO_USER_API_KEY)
    elif inputs.DOMINO_JOB_OP == "stop":
        job_stop(stop_job_url, project_id, inputs.DOMINO_JOB_ID, inputs.DOMINO_USER_API_KEY)
